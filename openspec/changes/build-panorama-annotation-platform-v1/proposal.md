...
- 明确首版非目标：不迁移当前 HOHONET/Label Studio 实验，不开放公共注册，不实现支付、聊天、CAD/BIM、桌面客户端、多房间 floor-plan reconstruction、worker-facing Manhattan A-line、云端在线推理、完全自动自适应路由或 Dataset/Model Release 管理；不把多数共识宣称为绝对真值。
...
- `annotation-contract`: 有序顶/底角点对、规范化坐标、seam、版本化元标签、三态 worker scope evidence、portal observation、派生 closure 边界与服务器校验。
