...
- [ ] 13.10 在生产 Supabase Dashboard 禁用 Data API 或从 exposed schema 移除 `public`，经 Django release migration 部署 13.9 后只读复核对象权限和 security advisor；不得用 Supabase migration history 形成第二套 DDL authority。（2026-08-13 已用一次性 break-glass migration 将生产有效表/序列/函数权限和危险默认授权降为 0、固定 21 个函数且 advisor 清零；仍待 Dashboard 设置确认及合并部署后由 Django `work.0028` 正式登记。）
