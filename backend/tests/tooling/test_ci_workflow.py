...
def test_ci_runs_media_immutability_against_postgresql() -> None:
    ...
    assert "backend/tests/test_activity_api.py" in workflow
    assert "backend/tests/test_analysis_jobs_postgres.py" in workflow
    ...
    assert "backend/tests/test_revision_immutability_postgres.py" in workflow
    ...
    assert "python backend/manage.py migrate activity --noinput" in workflow
